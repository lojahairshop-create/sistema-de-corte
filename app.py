...
CONFIG_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "config_orcamento.json")
...
# Pre-initialize session state from config
DEFAULT_CONFIG_DEFAULTS = {
    ...
    "p_inox": 32.0,
    "p_alum": 25.0,
    "em_nome": "2R CORTE LASER",
    "em_resp": "Wellington Rafael",
    "em_end": "Av. Alexandre José Barbosa, 215 - Jardim São Luiz II\nItatiba - SP, 13.253-080",
    "em_tel": "(11) 4524-3463 – Ramal: 219",
    "em_cel": "(11) 98994-4136",
    "em_email": "comercial@2rcortelaser.com.br",
    "incluir_termos": True,
    "cond_prazo": "7 Dias úteis após recebimento do pedido de compra",
    "cond_pgto": "A Combinar",
    "cond_minimo": 500.0,
    "cond_frete": "FOB",
    "cond_impostos": "ICMS INCLUSO - PIS/COFINS INCLUSO\nIPI: 3.25% A INCLUIR",
    "cond_coment": "Venda IPI - 3,25% / Benef. Isento",
    "nome_proj": "Orçamento 001",
    "nome_cli": "",
    "orcam": "",
    "tipo_venda": "Revenda",
    "cliente_end": "Singapura",
    "cliente_tel": "",
    "validade_dias": 7
}

for k, v in DEFAULT_CONFIG_DEFAULTS.items():
...
if "condicoes_texto" not in st.session_state:
    loaded_terms = cfg.get("condicoes_texto", DEFAULT_TERMS)
    terms_with_ids = []
    import uuid
    for term in loaded_terms:
        if isinstance(term, dict) and "id" in term and "texto" in term:
            terms_with_ids.append(term)
        elif isinstance(term, str):
            terms_with_ids.append({"id": str(uuid.uuid4())[:8], "texto": term})
    st.session_state.condicoes_texto = terms_with_ids
...
def sincronizar_itens_para_df():
    ...
    st.session_state.df_itens = pd.DataFrame(input_rows)

def on_grid_itens_edit():
    editor_state = st.session_state.get("grid_itens_editor", {})
    ...
    for row_idx_str, changes in edited_rows.items():
        ...
        if row_idx >= len(st.session_state.itens):
            continue
            
        # 1. Update st.session_state.df_itens in place
        for col_name, new_val in changes.items():
            st.session_state.df_itens.at[row_idx, col_name] = new_val
            
        # 2. Update st.session_state.itens in place
        orig_item = st.session_state.itens[row_idx]
        r = st.session_state.df_itens.iloc[row_idx]
        
        st.session_state.itens[row_idx] = {
...
def render_editor_grid(modo):
    ...
    if modo == "⚙️ Operações e Tempos":
        col_config["Material"] = None
        col_config["Espessura (mm)"] = None
        col_config["Largura (mm)"] = None
        col_config["Comprimento (mm)"] = None
        col_config["Perímetro (mm)"] = None
        col_config["Entradas"] = None
        col_config["Preço R$/kg"] = None
    elif modo == "📏 Geometria e Materiais":
        col_config["Setup (min)"] = None
        col_config["Dobra (min)"] = None
        col_config["Caldeiraria (min)"] = None
        col_config["Solda (min)"] = None
        col_config["Guilhotina (min)"] = None
        col_config["Usinagem (min)"] = None
        col_config["Montagem (min)"] = None
        col_config["Custo Extra (R$)"] = None
        col_config["Corte Override (min)"] = None
...
with st.sidebar:
    ...
    with st.expander("📊 Impostos (%)", expanded=True):
        tx_icms = st.number_input("ICMS", value=float(st.session_state.tx_icms), step=0.5, format="%.2f", key="tx_icms", on_change=save_s, args=("tx_icms",)) / 100
        tx_ipi = st.number_input("IPI", value=float(st.session_state.tx_ipi), step=0.5, format="%.2f", key="tx_ipi", on_change=save_s, args=("tx_ipi",)) / 100
        tx_pis = st.number_input("PIS", value=float(st.session_state.tx_pis), step=0.05, format="%.2f", key="tx_pis", on_change=save_s, args=("tx_pis",)) / 100
        tx_cofins = st.number_input("COFINS", value=float(st.session_state.tx_cofins), step=0.1, format="%.2f", key="tx_cofins", on_change=save_s, args=("tx_cofins",)) / 100
        tx_csll = st.number_input("CSLL", value=float(st.session_state.tx_csll), step=0.1, format="%.2f", key="tx_csll", on_change=save_s, args=("tx_csll",)) / 100
        tx_irpj = st.number_input("IRPJ", value=float(st.session_state.tx_irpj), step=0.1, format="%.2f", key="tx_irpj", on_change=save_s, args=("tx_irpj",)) / 100
    ...
    with st.expander("💰 Margem e Comissão", expanded=True):
        margem_lucro = st.number_input("Margem de Lucro (%)", value=float(st.session_state.margem), step=5.0, format="%.1f", key="margem", on_change=save_s, args=("margem",)) / 100
        taxa_comissao = st.number_input("Comissão (%)", value=float(st.session_state.comissao), step=0.5, format="%.1f", key="comissao", on_change=save_s, args=("comissao",)) / 100
        
    with st.expander("🛠️ Tarifas Hora-Máquina (R$/h)", expanded=False):
        tar_corte = st.number_input("Corte Laser", value=float(st.session_state.tar_corte), step=10.0, key="tar_corte", on_change=save_s, args=("tar_corte",))
        tar_setup = st.number_input("SET-UP", value=float(st.session_state.tar_setup), step=10.0, key="tar_setup", on_change=save_s, args=("tar_setup",))
        tar_dobra = st.number_input("Dobra", value=float(st.session_state.tar_dobra), step=10.0, key="tar_dobra", on_change=save_s, args=("tar_dobra",))
        tar_caldeiraria = st.number_input("Caldeiraria", value=float(st.session_state.tar_cald), step=10.0, key="tar_cald", on_change=save_s, args=("tar_cald",))
        tar_solda = st.number_input("Solda", value=float(st.session_state.tar_solda), step=10.0, key="tar_solda", on_change=save_s, args=("tar_solda",))
        tar_guilhotina = st.number_input("Guilhotina", value=float(st.session_state.tar_guil), step=10.0, key="tar_guil", on_change=save_s, args=("tar_guil",))
        tar_usinagem = st.number_input("Usinagem Interna", value=float(st.session_state.tar_usin), step=10.0, key="tar_usin", on_change=save_s, args=("tar_usin",))
        tar_montagem = st.number_input("Montagem", value=float(st.session_state.tar_mont), step=10.0, key="tar_mont", on_change=save_s, args=("tar_mont",))
        
    with st.expander("⚙️ Material e Chapa Padrão", expanded=False):
        chapa_l_pad = st.number_input("Largura Chapa (mm)", value=float(st.session_state.ch_l), step=100.0, key="ch_l", on_change=save_s, args=("ch_l",))
        chapa_c_pad = st.number_input("Comprimento Chapa (mm)", value=float(st.session_state.ch_c), step=100.0, key="ch_c", on_change=save_s, args=("ch_c",))
        preco_aco = st.number_input("Preço Aço Carbono (R$/kg)", value=float(st.session_state.p_aco), step=0.5, format="%.2f", key="p_aco", on_change=save_s, args=("p_aco",))
        preco_inox = st.number_input("Preço Inox (R$/kg)", value=float(st.session_state.p_inox), step=1.0, format="%.2f", key="p_inox", on_change=save_s, args=("p_inox",))
        preco_alum = st.number_input("Preço Alumínio (R$/kg)", value=float(st.session_state.p_alum), step=1.0, format="%.2f", key="p_alum", on_change=save_s, args=("p_alum",))
    ...
    with st.expander("📄 Configurações do PDF (Emissor/Logo)", expanded=False):
        ...
        emissor_nome = st.text_input("Nome da Empresa", value=st.session_state.em_nome, key="em_nome", on_change=save_s, args=("em_nome",))
        emissor_resp = st.text_input("Responsável / Vendedor", value=st.session_state.em_resp, key="em_resp", on_change=save_s, args=("em_resp",))
        emissor_end = st.text_area("Endereço", value=st.session_state.em_end, key="em_end", on_change=save_s, args=("em_end",))
        emissor_tel = st.text_input("Telefone", value=st.session_state.em_tel, key="em_tel", on_change=save_s, args=("em_tel",))
        emissor_cel = st.text_input("Celular", value=st.session_state.em_cel, key="em_cel", on_change=save_s, args=("em_cel",))
        emissor_email = st.text_input("E-mail", value=st.session_state.em_email, key="em_email", on_change=save_s, args=("em_email",))

    with st.expander("📜 Termos e Condições do PDF", expanded=False):
        incluir_termos = st.checkbox("Incluir Termos e Condições no PDF", value=st.session_state.incluir_termos, key="incluir_termos", on_change=save_s, args=("incluir_termos",))
        
        cond_prazo = st.text_input("Prazo de Entrega", value=st.session_state.cond_prazo, key="cond_prazo", on_change=save_s, args=("cond_prazo",))
        cond_pgto = st.text_input("Forma de Pagamento", value=st.session_state.cond_pgto, key="cond_pgto", on_change=save_s, args=("cond_pgto",))
        cond_minimo = st.number_input("Pedido Mínimo (R$)", value=float(st.session_state.cond_minimo), step=50.0, key="cond_minimo", on_change=save_s, args=("cond_minimo",))
        cond_frete = st.text_input("Frete", value=st.session_state.cond_frete, key="cond_frete", on_change=save_s, args=("cond_frete",))
        cond_impostos = st.text_area("Impostos Descrição", value=st.session_state.cond_impostos, key="cond_impostos", on_change=save_s, args=("cond_impostos",))
        cond_comentarios = st.text_area("Mensagem de Observações / Totais", value=st.session_state.cond_coment, key="cond_coment", on_change=save_s, args=("cond_coment",))
        
        if incluir_termos:
            st.markdown("**Texto das Condições Gerais:**")
            
            def save_term_change(tid, key_name):
                for t in st.session_state.condicoes_texto:
                    if t['id'] == tid:
                        t['texto'] = st.session_state[key_name]
                        break
                save_config({"condicoes_texto": st.session_state.condicoes_texto})

            for i, term_dict in enumerate(st.session_state.condicoes_texto):
                tid = term_dict['id']
                ttext = term_dict['texto']
                key_name = f"term_input_{tid}"
                ...
                with col_text:
                    st.text_area(
                        ...
                        value=ttext,
                        key=key_name,
                        on_change=save_term_change,
                        args=(tid, key_name)
                    )
                with col_btn:
                    st.markdown("<div style='height: 28px;'></div>", unsafe_allow_html=True) # spacer
                    if st.button("🗑️", key=f"remove_term_{tid}", help="Remover este termo"):
                        st.session_state.condicoes_texto = [t for t in st.session_state.condicoes_texto if t['id'] != tid]
                        save_config({"condicoes_texto": st.session_state.condicoes_texto})
                        st.rerun()
            
            if st.button("➕ Adicionar Novo Termo"):
                import uuid
                st.session_state.condicoes_texto.append({"id": str(uuid.uuid4())[:8], "texto": "Novo termo adicionado. Edite-me."})
                save_config({"condicoes_texto": st.session_state.condicoes_texto})
                st.rerun()
...
def gerar_pdf_com_ajuste():
    ...
    _prazos = {
        ...
        'condicoes_texto': [t['texto'] for t in ss.condicoes_texto],
        'incluir_termos': ss.get("incluir_termos", True)
    }
...
with pc1:
    nome_projeto = st.text_input("Projeto / Orçamento Nº", value=st.session_state.nome_proj, key="nome_proj", on_change=save_s, args=("nome_proj",))
with pc2:
    nome_cliente = st.text_input("Cliente", value=st.session_state.nome_cli, key="nome_cli", on_change=save_s, args=("nome_cli",))
with pc3:
    orcamentista = st.text_input("Orçamentista", value=st.session_state.orcam, key="orcam", on_change=save_s, args=("orcam",))
with pc4:
    tipo_venda = st.selectbox("Tipo de Venda", ["Revenda", "Venda Direta", "Industrialização"], index=["Revenda", "Venda Direta", "Industrialização"].index(st.session_state.tipo_venda), key="tipo_venda", on_change=save_s, args=("tipo_venda",))

pc5, pc6, pc7, pc8 = st.columns([2.5, 1.5, 1, 1])
with pc5:
    cliente_endereco = st.text_input("Endereço do Cliente", value=st.session_state.cliente_end, key="cliente_end", on_change=save_s, args=("cliente_end",))
with pc6:
    cliente_telefone = st.text_input("Telefone do Cliente", value=st.session_state.cliente_tel, key="cliente_tel", on_change=save_s, args=("cliente_tel",))
with pc7:
    data_criacao_input = st.date_input("Data de Criação", value=datetime.now())
with pc8:
    validade_dias = st.number_input("Validade (dias)", min_value=1, value=int(st.session_state.validade_dias), key="validade_dias", on_change=save_s, args=("validade_dias",))
...
with tab_manual:
    ...
    with st.form("form_peca_manual", clear_on_submit=False):
        ...
        if submitted:
            ...
            # Clear manual form inputs from session state so they reset to default values on rerun
            form_keys = [
                "m_desc", "m_qtd", "m_mat", "m_espe", "m_larg", "m_compr", 
                "m_perim", "m_entr", "m_pr_kg", "m_setup", "m_dobra", 
                "m_calde", "m_solda", "m_guil", "m_usin", "m_mont", "m_extra"
            ]
            for k in form_keys:
...
if st.session_state.itens:
    ...
    with tab_editor:
        ...
        # Filtro de colunas para otimizar visualização e remover barras de rolagem
        modo_colunas = st.radio(
            "Visualização de Colunas:",
            ["⚙️ Operações e Tempos", "📏 Geometria e Materiais", "👁️ Mostrar Tudo"],
            horizontal=True,
            index=0,
            key="modo_colunas_planilha"
        )
        
        # Render the interactive grid wrapped in st.fragment to prevent full page scroll-to-top jumps
        render_editor_grid(modo_colunas)
